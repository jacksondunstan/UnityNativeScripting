...
/// <summary>
/// Code generator that reads a JSON file and outputs C# and C++ code bindings
/// so C++ can call managed functions.
/// 
/// Supports:
/// * Constructors
...
{
	// Disable unused field types. JsonUtility actually uses them, but it does
	// so with reflection.
	...
	static void Generate(bool dryRun)
	{
		...
		StringBuilder csharpInitParams = stringBuilders.CsharpInitParams;
		StringBuilder csharpDelegateTypes = stringBuilders.CsharpDelegateTypes;
		StringBuilder csharpInitCall = stringBuilders.CsharpInitCall;
		StringBuilder csharpFunctions = stringBuilders.CsharpFunctions;
		StringBuilder cppFunctionPointers = stringBuilders.CppFunctionPointers;
		StringBuilder cppTypeDeclarations = stringBuilders.CppTypeDeclarations;
		StringBuilder cppTypeDefinitions = stringBuilders.CppTypeDefinitions;
		StringBuilder cppMethodDefinitions = stringBuilders.CppMethodDefinitions;
		StringBuilder cppInitParams = stringBuilders.CppInitParams;
		StringBuilder cppInitBody = stringBuilders.CppInitBody;
		StringBuilder tempStrBuilder = stringBuilders.TempStrBuilder;
		foreach (JsonAssembly jsonAssembly in doc.Assemblies)
		{
			...
			foreach (JsonType jsonType in jsonAssembly.Types)
			{
				...
				{
					PropertyInfo property = type.GetProperty(jsonPropertyName);
				...
				{
					Type[] paramTypes = GetTypes(jsonMethod.Types, assembly);
		...
		if (dryRun)
		{
			PrintStringBuilder("C# init params", csharpInitParams);
			PrintStringBuilder("C# delegates", csharpDelegateTypes);
			PrintStringBuilder("C# init call", csharpInitCall);
			PrintStringBuilder("C# functions", csharpFunctions);
			PrintStringBuilder("C++ function pointers", cppFunctionPointers);
			PrintStringBuilder("C++ type declarations", cppTypeDeclarations);
			PrintStringBuilder("C++ type definitions", cppTypeDefinitions);
			PrintStringBuilder("C++ method definitions", cppMethodDefinitions);
			PrintStringBuilder("C++ init params", cppInitParams);
			PrintStringBuilder("C++ init body", cppInitBody);
		}
		else
		{
		...
		StringBuilders stringBuilders)
	{
		...
		stringBuilders.TempStrBuilder.Append(fieldName, 1, fieldName.Length-1);
